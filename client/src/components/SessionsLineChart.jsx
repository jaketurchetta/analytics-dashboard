...
export default class SessionsLineChart extends React.Component {
  ...
  updateScale(props) {
    ...
    const yDomain = props.yDomain || [0, d3.max(data, d => {
      console.log(d)
      return props.yFn(d.views)
    })]
    console.log(yDomain)
  ...
  render() {
    ...
    const metaData = {
      ...
      xSlide: -xScale(this.props.xFn(this.props.sessionPageViews[0].views))
    }
    const plotData = {
      plotData: this.props.sessionPageViews.map((d, i) => {
        return {
          id: i,
          data: d.views,
          x: xScale(this.props.xFn(d.views)),
          y: yScale(this.props.yFn(d.views))
        }
      })
    }
